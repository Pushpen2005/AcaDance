// Enhanced animations hooks (JavaScript version for compatibility)
import { useEffect, useRef, useCallback } from 'react';

/**
 * Custom hook for advanced 3D animations and interactivity
 */
export function useAdvanced3D(options = {}) {
 ...
 */
export function useScrollAnimation(animationType = 'fadeInUp', options = {}) {
  const elementRef = useRef(null);
...
export function useParticleEffect(options = {}) {
  ...
  const triggerParticles = useCallback((event) => {
    if (!containerRef.current) return;
